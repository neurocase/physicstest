...
var bulMagnitude = physicstest.Vector{0,0}
...
func fireCannon(){
	...
	bulMagnitude.X = bulMagspeed * math.Cos(bulRadian);
	bulMagnitude.Y = bulMagspeed * math.Sin(bulRadian);
...
func (game *Game) Draw(window *glfw.Window) {
	...
	// UNUSED CODE MAKES NON-EXISTANT ENEMIES FACE PLAYER
		...
		bullet.Xpos += bulMagnitude.X
		bullet.Ypos += bulMagnitude.Y
